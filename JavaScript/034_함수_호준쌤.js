...
안녕()              // 10       // '10'
console.log(String(안녕()) + String(안녕())); // 10 + '10' => '1010'
...
function 안녕(){
  ...
  return
  // return을 만나게 되면, 아래 코드는 실행하지 않습니다.
  .
  .
  .
  .
...
함수4('Gold', true, true, true, '대화방 전체 백업 가능', true); // 이런식으로 함수를 호출하게 되면 가독성이 떨어진다.
...
//// 서동현님 글 //// --> 지금 함수가 아규먼트가 있거나 없거나 실행될 수 있게 하는 코드입니다.
// 객체가 매핑이 안됐기때문에 객체구조는 무시되고 그대로 a=2, b=1, c=3만 실행되어 리턴된 것이다.
function 함수({
  a = 2, 
  b = 1, 
  c = 3
...
reverse('hello') |      false      | reverse(txt.slice(1)) + 'h'  = 'olloh'
reverse('ello')  |      false      | reverse(txt.slice(1)) + 'e'  = 'ollo'
