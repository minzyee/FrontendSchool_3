...
// return:  반환 / return 구문을 만나면 함수는 종료되고, 자신이 있던 자리의 값을 반환한다.
function 안녕(파라미터){ // 파라미터: 함수 내부에서 선언된 변수
  console.log(파라미터);
  console.log('hello world');
  return 100; // 얘가 있던 자리에 100을 넣어라. 자신을 불러온 자리에 값을 대입함
}
...
console.log(안녕); // 문법적 오류가 아니다. 함수 자체를 출력해준다.

// 콘솔로그의 리턴값은 undefined이다.
console.log(String(console.log('hello')) + String(console.log('hello'))) // undefinedundefined

// return은 반환값
// console.log()는 출력해줌
// 출력값과 반환값은 다르다...
// 그렇다면 함수의 연산과 관계없이 리턴값을 엉뚱한걸 명시하면
// 함수 바깥에서 함수를 호출했을때 엉뚱한 리턴값만 얻게 되는건가요? 그렇습니다.
...
//////////////함수를 사용하는 이유/////////////////
// 1. 재사용성 -> 재사용하지 않을 코드는 함수로 정의하나요? -> 코드마다 다르지만 가능하면 함수로 만드는 것을 권합니다.
// 2. 유지보수 -> 3번 사항 + 변수의 스코프 제한
// 3. 코드의 간결성 -> 구조 파악 용이하다.
// 4. (참고만해주세요)추상화로 개발하는 것이 객체지향 프로그래밍(OOP, Object Oriented Programming) 언어의 언어 철학에 맞다.
...
// 콘솔창의 기능입니다. 마지막 라인에 한하여 console.log()를 찍지 않아도 return값을 console창에 출력해줍니다.
console.log(함수1(10, 20, 30)); // 60
console.log(함수1(10, 20, 50)); // 80
// undefined. --> 콘솔창의 리턴값은 undefined이다.

// 필요 이상의 아규먼트를 넣었을 때는?
함수1(10, 20, 30, 40); // Error를 뿜지않는다. 60이 나온다.
...
console.log(함수3(c = 1000)); // 답: 1050 / 1030이 되어야 하는거 아닌가요?
console.log(함수3(a = 1000, c = 1000)); // 답: 3030 / 3020이 되어야 하는거 아닌가요?
// 값이 순서대로 들어갔습니다.
...
////// 재귀 함수 //////
// 재귀함수: 내가 나를 호출하는 함수
// 나무위키: https://namu.wiki/w/%EC%9E%AC%EA%B7%80%ED%95%A8%EC%88%98
// 재귀함수로 구현 가능한 것은, 반복문으로도 구현이 가능하다.
// 재귀함수 <-> 반복문
// 재귀함수는 어려워요. 손으로 그려보고 테이블로도 그려보고 해야합니다. 이해하는데 오래 걸립니다.